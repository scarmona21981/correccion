...
const qwwAccumulator_1 = require("../utils/qwwAccumulator");
const assertClose = (value, expected, tol, label) => {
    if (Math.abs(value - expected) > tol) {
        throw new Error(label + ': expected ' + expected.toFixed(6) + ' +/- ' + tol + ', got ' + value.toFixed(6));
    }
};
const makeChamber = (id) => ({
    id,
    userDefinedId: id,
    x: 0,
    y: 0,
...
const makePipe = (id, startNodeId, endNodeId) => ({
    id,
    userDefinedId: id,
    x1: 0,
    y1: 0,
...
const resultI = (0, qwwAccumulator_1.calculateQwwAccumulation)([c1, c2], [p1], 'I');
...
if (!outC1_I || !outC2_I)
    throw new Error('Missing chamber results for system I.');
...
if (!outPipeI)
    throw new Error('Missing pipe result for system I.');
...
const resultII = (0, qwwAccumulator_1.calculateQwwAccumulation)([c1, c2], [p1], 'II');
const ownC1_II = Math.sqrt((0.5 * 0.5 * 120) / 60);
const outC1_II = resultII.chambers.find(c => c.id === 'C1');
if (!outC1_II)
    throw new Error('Missing chamber C1 for system II.');
...
const cycleResult = (0, qwwAccumulator_1.calculateQwwAccumulation)([c1, c2], [p1, cyclePipe], 'I');
